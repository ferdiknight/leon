...
public class MultiNetworkTypeahead<E extends Element> implements NetworkTypeahead<E>
{
    ...
    public Collector<E> searchNetwork(int uid, String[] terms, Collector<E> collector, NetworkTypeaheadContext context)
    {
        ...
        for(NetworkTypeahead<E> nt : typeaheads)
        {
            if(nt.getRange().has(uid))
            {
                return nt.searchNetwork(uid, terms, collector, context);
            }
        }
        
        return new SortedCollector<E>();
        
        
//        List<NetworkTypeaheadTask<E>> taskList = new ArrayList<NetworkTypeaheadTask<E>>(typeaheads.size());
...
//                NetworkTypeahead<E> ta = typeaheadMap.get(source);
//                if (c != null && ta != null)
//                {
//                    taskList.add(new NetworkTypeaheadTask<E>(ta,uid,terms,collector,context));
//                }
//            }
//        }
//        else
...
//            for (NetworkTypeahead<E> ta : typeaheads)
//            {
//                Collector<E> c = collector.newInstance();
//                multiCollector.putCollector(ta.getName(), c);
//                taskList.add(new NetworkTypeaheadTask<E>(ta, uid, terms, c, context));
//            }
//        }
//        
...
//            for (NetworkTypeaheadTask<E> t : taskList)
//            {
...
//        return multiCollector == null ? collector : multiCollector;
    ...
    public Collector<E> search(int uid, String[] terms, Collector<E> collector, long timeoutMillis)
    {
        for(NetworkTypeahead<E> nt : typeaheads)
        {
            if(nt.getRange().has(uid))
            {
                return nt.search(uid, terms, collector, timeoutMillis);
            }
        }
